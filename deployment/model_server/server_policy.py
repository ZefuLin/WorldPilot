...
def main(args) -> None:
    ...
    # Override cosmos server port if specified (for multi-GPU parallel eval)
    if args.cosmos_port is not None and hasattr(vla, "cosmos_port"):
        vla.cosmos_port = args.cosmos_port
        logging.info("Cosmos server port overridden to %d", args.cosmos_port)
...
def build_argparser():
    ...
    parser.add_argument("--cosmos_port", type=int, default=None,
                        help="Override cosmos server port from model config (for multi-GPU parallel eval)")
    return parser

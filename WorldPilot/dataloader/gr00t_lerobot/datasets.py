...
from WorldPilot.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_timestamps

from WorldPilot.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from WorldPilot.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatistics,
...
from WorldPilot.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Precomputed Cosmos cache directory (set externally or via data_cfg)
        self._cosmos_cache_dir = self.data_cfg.get("cosmos_cache_dir", None) if self.data_cfg else None
        self._cosmos_episode_cache = {}  # LRU: {trajectory_id: npz_data_dict}
        self._cosmos_cache_max = 4  # max episodes in memory (reduced from 8 to lower worker RSS)
    ...
    def _get_missing_cosmos_cache_ids(self) -> list[int]:
        if not self._cosmos_cache_dir:
            return []
        cache_ds_dir = Path(self._cosmos_cache_dir) / self.dataset_name
        missing_ids = []
        for trajectory_id in self.trajectory_ids:
            npz_path = cache_ds_dir / f"episode_{int(trajectory_id):06d}.npz"
            if not npz_path.exists():
                missing_ids.append(int(trajectory_id))
        return sorted(missing_ids)

    def _print_cosmos_cache_summary(self, total_steps: int, missing_ids: list[int]) -> None:
        if not self._cosmos_cache_dir:
            return
        print("=" * 72)
        print(f"[Cosmos Cache Summary] dataset={self.dataset_name}")
        print(f"  total_episodes: {len(self.trajectory_ids)}")
        print(f"  missing_cache_episodes: {len(missing_ids)}")
        print(f"  included_steps: {total_steps}")
        if missing_ids:
            print(f"  skipped_trajectory_ids: {missing_ids}")
        print("=" * 72)
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        if steps_path.exists():
            try:
                ...
                if cached_data.get("config_key") == config_key:
                    skipped_ids = cached_data.get("skipped_cosmos_cache_ids", [])
                    if is_main():
                        self._print_cosmos_cache_summary(
                            total_steps=len(cached_data["steps"]),
                            missing_ids=skipped_ids,
                        )
        ...
        if is_main():
            ...
            cache_data = {
                ...
                "skipped_cosmos_cache_ids": getattr(self, "_last_skipped_cosmos_cache_ids", []),
            }
    ...
    def _get_steps_config_key(self) -> str:
        ...
        cosmos_cache_signature = None
        if self._cosmos_cache_dir:
            cache_ds_dir = Path(self._cosmos_cache_dir) / self.dataset_name
            if cache_ds_dir.exists():
                cache_files = sorted(p.name for p in cache_ds_dir.glob("episode_*.npz"))
                cosmos_cache_signature = hashlib.md5("|".join(cache_files).encode()).hexdigest()[:12]
            else:
                cosmos_cache_signature = "missing_cache_dir"
        config_dict = {
            "steps_index_schema_version": 2,
            "delete_pause_frame": self.delete_pause_frame,
            "dataset_name": self.dataset_name,
            "cosmos_cache_dir": self._cosmos_cache_dir,
            "cosmos_cache_signature": cosmos_cache_signature,
        }
    ...
    def _get_all_steps_single_process(self) -> list[tuple[int, int]]:
        ...
        skipped_cosmos_cache = 0
        skipped_cosmos_cache_ids: list[int] = []
        processed_trajectories = 0

        cache_ds_dir = Path(self._cosmos_cache_dir) / self.dataset_name if self._cosmos_cache_dir else None
        ...
        for trajectory_id, trajectory_length in tqdm(zip(self.trajectory_ids, self.trajectory_lengths), total=len(self.trajectory_ids), desc="Getting All Step"):
            if cache_ds_dir is not None:
                npz_path = cache_ds_dir / f"episode_{trajectory_id:06d}.npz"
                if not npz_path.exists():
                    skipped_cosmos_cache += 1
                    skipped_cosmos_cache_ids.append(int(trajectory_id))
                    continue
        ...
        self._last_skipped_cosmos_cache_ids = skipped_cosmos_cache_ids

        # Print summary statistics
        print(
            f"Single-process summary: Processed {processed_trajectories} trajectories, "
            f"skipped {skipped_trajectories} invalid trajectories, skipped {skipped_cosmos_cache} trajectories without cosmos cache"
        )
        print(f"Total steps: {len(all_steps)} from {len(self.trajectory_ids)} trajectories")
        self._print_cosmos_cache_summary(
            total_steps=len(all_steps),
            missing_ids=skipped_cosmos_cache_ids,
        )
    ...
    def __getitem__(self, index: int, **kwargs) -> dict:
        ...
        if sample is None:
            # Cosmos cache missing for this episode — resample (with depth limit)
            _resample_depth = kwargs.get("_resample_depth", 0)
            if _resample_depth >= 10:
                raise RuntimeError(
                    f"Cosmos cache missing for 10 consecutive resamples in {self.dataset_name}. "
                    f"Check that cosmos_cache_dir has npz files for this dataset."
                )
            logger.warning(
                "⚠️ Cosmos cache missing, resampling (%d/10): dataset=%s ep=%d frame=%d",
                _resample_depth + 1, self.dataset_name, trajectory_id, base_index,
            )
            new_index = random.randint(0, len(self) - 1)
            return self.__getitem__(new_index, _resample_depth=_resample_depth + 1)
        return sample
    
    def _load_cosmos_episode(self, cache_dir: str, trajectory_id: int):
        """Load precomputed Cosmos data for an episode, with simple LRU cache."""
        if trajectory_id in self._cosmos_episode_cache:
            return self._cosmos_episode_cache[trajectory_id]
        npz_path = os.path.join(cache_dir, self.dataset_name, f"episode_{trajectory_id:06d}.npz")
        if not os.path.exists(npz_path):
            return None
        data = dict(np.load(npz_path))
        while len(self._cosmos_episode_cache) >= self._cosmos_cache_max:
            oldest = next(iter(self._cosmos_episode_cache))
            evicted = self._cosmos_episode_cache.pop(oldest)
            del evicted
        self._cosmos_episode_cache[trajectory_id] = data
        return data
    ...
    def _pack_sample(self, data: dict, trajectory_id: int = None, base_index: int = None) -> dict:
        ...
        # ---- Precomputed Cosmos data ----
        cosmos_cache_dir = getattr(self, "_cosmos_cache_dir", None)
        if cosmos_cache_dir is not None and trajectory_id is not None and base_index is not None:
            ep_data = self._load_cosmos_episode(cosmos_cache_dir, trajectory_id)
            if ep_data is None:
                return None  # signal caller to resample
            sample["cosmos_future_latents"] = ep_data["future_image_latents"][base_index]  # (2,16,28,28) float16
            sample["cosmos_action_chunk"] = ep_data["action_chunk"][base_index]            # (16,7) float16
            sample["cosmos_value"] = ep_data["value"][base_index]                          # scalar float32
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Counter for periodic malloc_trim to prevent heap fragmentation
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if sample is None:
                    # Cosmos cache missing — resample
                    logger.warning(
                        "⚠️ Cosmos cache missing, skipping: dataset=%s ep=%d frame=%d",
                        dataset.dataset_name, trajectory_id, step,
                    )
                    index = random.randint(0, len(self) - 1)
                    continue
                ...
                # Periodic malloc_trim to prevent heap fragmentation
                self._getitem_call_count += 1
                if self._getitem_call_count % 200 == 0:
                    try:
                        import ctypes
                ...
                # Break traceback references to stack frames so C-level resources such as video readers can be released.
                e.__traceback__ = None
                last_exception = e
                gc.collect()  # Trigger GC immediately to release av codec contexts / FFmpeg buffers.
                try:
                    import ctypes
                    ctypes.CDLL("libc.so.6").malloc_trim(0)  # Return fragmented free memory to the OS.
                except Exception:
                    pass

...
def generate_dataset_mixture(
    ...
    for root_idx, (abs_root, rel_root, mapping_json) in enumerate(
        ...
        if not os.path.exists(abs_root):
            import warnings
            warnings.warn(f"Root path does not exist, skipping: {abs_root}")
            continue

        if not os.path.isfile(mapping_json):
            import warnings
            warnings.warn(f"Mapping json does not exist, skipping: {mapping_json}")
            continue
    ...
    if total_final <= 0:
        import warnings
        warnings.warn(f"Mixture '{mixture_name}': all roots skipped or empty, returning empty mixture.")
        return {mixture_name: []}

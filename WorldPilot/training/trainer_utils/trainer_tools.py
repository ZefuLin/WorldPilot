...
class TrainerUtils:
    ...
    def _get_latest_checkpoint(self, checkpoint_dir):
        ...
        # Collect all checkpoints matching the naming rule, supporting .pt and .safetensors.
        checkpoints = [
            f for f in os.listdir(checkpoint_dir) 
            if re.match(r"steps_(\d+)_(?:pytorch_model\.pt|model\.safetensors)$", f)
            and os.path.isfile(os.path.join(checkpoint_dir, f))  # Make sure it is a file.
        ]
        ...
        # Sort by step count and get the latest checkpoint.

...
from WorldPilot.model.modules.action_model.flow_matching_head.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)

from WorldPilot.model.modules.action_model.flow_matching_head.cross_attention_dit import DiT
...
class FlowmatchingActionHead(nn.Module):
    ...
    def forward(self, vl_embs: torch.Tensor, actions: torch.Tensor, state: torch.Tensor = None, encoder_attention_mask=None, action_mask: torch.Tensor = None, cosmos_action_hint: torch.Tensor = None):
        """
        ...
        # ========== Step 7: Concatenate state, cosmos hint, future tokens, and action features ==========
        # Sequence order (0401): [state(opt)] + [cosmos_hints(opt)] + [future_tokens] + [action_features]
    ...
    @torch.no_grad()
    def predict_action(self, vl_embs: torch.Tensor, state: torch.Tensor = None, cosmos_action_hint: torch.Tensor = None) -> torch.Tensor:
        """
        ...
        for t in range(num_steps):
            ...
            # Same order as training forward(): [state] + [cosmos_hints] + [future_tokens] + [action_features]
            future_tokens = self.future_tokens.weight.unsqueeze(0).expand(vl_embs.shape[0], -1, -1)
            parts = []
            if state_features is not None:
                parts.append(state_features)

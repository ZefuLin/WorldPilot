...
_ACTION_TOKEN_MAX = 153716 # here only for fast_tokenizer, see WorldPilot/model/modules/vlm/tools/add_qwen_special_tokens/README.md
...
class _QWen3_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        env_model_id = (
            os.environ.get("BASE_VLM")
            or os.environ.get("WORLDPILOT_BASE_VLM")
        )
        if env_model_id:
            logger.info("Overriding Qwen3-VL base model from environment: %s", env_model_id)
        ...
        try:
            ...
            logging.warning(
                "FlashAttention2 is unavailable for `%s`, falling back to SDPA: %s",
                model_id,
                exc,
            )
            model = Qwen3VLForConditionalGeneration.from_pretrained(
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        if solutions is not None: #  here only for fast_tokenizer now. 
            ...
            action_token_max = _ACTION_TOKEN_MAX # here only for fast_tokenizer, see WorldPilot/model/modules/vlm/tools/add_qwen_special_tokens/README.md
            ...
            for i in range(labels.size(0)):
                ...
                if nonzero_indices.numel() > 0:
                    ...
                    seq[:] = IGNORE_INDEX
                    RuntimeWarning (f"action token are on in yout tokenizer, plz see WorldPilot/model/modules/vlm/tools/add_qwen_special_tokens/README.md.")
